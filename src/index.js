...
const analyze = (css) => {
  ...
  const borders = new CountableCollection();
  ...
  walk(ast, function (node) {
    switch (node.type) {
      case "Atrule": {
        ...
        if (atRuleName === "font-face") {
          ...
          node.block.children.forEach(
            ...
            (descriptor) =>
              (descriptors[descriptor.property] = stringifyNode(
                descriptor.value
              ))
          );
      ...
      case "Rule": {
        const numSelectors = node.prelude.children
          ? node.prelude.children.size
          : 0;
        const numDeclarations = node.block.children
          ? node.block.children.size
          : 0;
      ...
      case "Selector": {
        ...
        const { specificity, complexity, isId, isA11y } =
          analyzeSpecificity(node);
        ...
        if (
          minSpecificity !== undefined &&
          compareSpecificity(minSpecificity, specificity) < 0
        ) {
          minSpecificity = specificity;
        }

        if (
          maxSpecificity !== undefined &&
          compareSpecificity(maxSpecificity, specificity) > 0
        ) {
          maxSpecificity = specificity;
        }
      ...
      case "Value": {
        ...
        if (isProperty("z-index", property)) {
          ...
          isProperty("transition", property) ||
          isProperty("animation", property)
        ) {
          ...
          isProperty("animation-duration", property) ||
          isProperty("transition-duration", property)
        ) {
          durations.push(stringifyNode(node));
          break;
        } else if (
          isProperty("transition-timing-function", property) ||
          isProperty("animation-timing-function", property)
        ) {
  ...
  const complexityCount = new CountableCollection(
    selectorComplexities.toArray()
  ).count();
  ...
  return {
    ...
    atrules: {
      ...
      keyframes: assign(keyframes.count(), {
        prefixed: assign(prefixedKeyframes.count(), {
          ratio:
            keyframes.size() === 0
              ? 0
              : prefixedKeyframes.size() / keyframes.size(),
        }),
      }),
    ...
    rules: {
      ...
      sizes: assign(ruleSizes.aggregate(), {
      ...
      selectors: assign(selectorsPerRule.aggregate(), {
      ...
      declarations: assign(declarationsPerRule.aggregate(), {
    ...
    selectors: {
      ...
      uniquenessRatio:
        totalSelectors === 0 ? 0 : totalUniqueSelectors / totalSelectors,
      specificity: {
        ...
        median: [
          specificitiesA.median,
          specificitiesB.median,
          specificitiesC.median,
        ],
        items: specificities,
      ...
      complexity: assign(selectorComplexities.aggregate(), complexityCount, {
        items: selectorComplexities.toArray(),
      }),
      id: assign(ids.count(), {
        ratio: totalSelectors === 0 ? 0 : ids.size() / totalSelectors,
      }),
      accessibility: assign(a11y.count(), {
    ...
    declarations: {
      ...
      uniquenessRatio:
        totalDeclarations === 0
          ? 0
          : totalUniqueDeclarations / totalDeclarations,
      ...
      importants: {
        ...
        ratio:
          totalDeclarations === 0
            ? 0
            : importantDeclarations / totalDeclarations,
        inKeyframes: {
          total: importantsInKeyframes,
          ratio:
            importantDeclarations === 0
              ? 0
              : importantsInKeyframes / importantDeclarations,
        },
      },
    },
    properties: assign(properties.count(), {
      prefixed: assign(propertyVendorPrefixes.count(), {
        ratio:
          properties.size() === 0
            ? 0
            : propertyVendorPrefixes.size() / properties.size(),
      }),
      custom: assign(customProperties.count(), {
        ratio:
          properties.size() === 0
            ? 0
            : customProperties.size() / properties.size(),
      }),
      browserhacks: assign(propertyHacks.count(), {
        ratio:
          properties.size() === 0
            ? 0
            : propertyHacks.size() / properties.size(),
      }),
    }),
    values: {
      colors: colors.count(),
